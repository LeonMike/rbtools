...
export default class RBToolsButton extends Disposable {
  ...
  onClick(event, actionType, action, params) {
    if (actionType == 'atomCommand') {
      ...
      var currentFolder = path.dirname(atom.workspace.getActiveTextEditor().getPath());
      var currentSelectedElementInTreeViewer = document.querySelector('.tree-view .selected').getPath();
      var currentProjectFolder = atom.project.relativizePath(currentSelectedElementInTreeViewer)[0];
